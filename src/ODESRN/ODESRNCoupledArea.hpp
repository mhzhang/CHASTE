...
AN ODESRN SYSTEM COUPLED WITH THE AREA OF THE CORRESPONDING CELL
Author: MoHan Zhang
...
public:
                              ...
                              std::vector<double>& rDY)
    {
        ...
        rDY[0] = ((0.1 + beta * (pow(rY[2],10)/(pow(rY[1],10)+pow(rY[2],10))) + 1.5*(pow(rY[0],4)/(1+pow(rY[0],4))))*(2-rY[0])-rY[0])*1; //G
        rDY[1] = (-0.1*(rY[1]-1.15*(1-0.75*(pow(rY[0],4)/(pow(0.3,4)+pow(rY[0],4))))))*1; //Target Area
		rDY[2] = 0; //Area, previously set to 0.866025
    }
};
