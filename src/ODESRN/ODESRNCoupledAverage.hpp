...
AN ODESRN SYSTEM TO BE USED WITH COUPLING NEIGHBOURING CELLS' AVERAGE RAC AND RHO CHEMICAL CONCENTRATIONS.
Author: MoHan Zhang
...
public:
                              ...
                              std::vector<double>& rDY)
    {
		...
		double IW=3;
		double IF=3;
        rDY[0] = IF/(1+pow(rY[3],m))-db*rY[0];
        rDY[1] = IW/(1+pow(rY[2],n))-da*rY[1];
		rDY[2] = 0; //Average Rac
		rDY[3] = 0; //Average Rho
...
void OdeSystemInformation<ODESRN>::Initialise()
{
    ...
    this->mVariableNames.push_back("Avg_Rac");
    this->mVariableUnits.push_back("dimensionless");
    this->mInitialConditions.push_back(10);
	
    this->mVariableNames.push_back("Avg_Rho");
    this->mVariableUnits.push_back("dimensionless");
    this->mInitialConditions.push_back(10);
...
public:
    ...
    ODESrnModel()
        : AbstractOdeSrnModel(4, boost::shared_ptr<AbstractCellCycleModelOdeSolver>())
    {
    ...
    void SimulateToCurrentTime()
    {
		...
		mpCell->GetCellData()->SetItem("Avg_Rac",mpOdeSystem->rGetStateVariables()[2]);
		mpCell->GetCellData()->SetItem("Avg_Rho",mpOdeSystem->rGetStateVariables()[3]);
    }
